...
char	*parse_flag_identifier(char *to_parse)
{
	...
	if (count >= 3)
		error_exit(NULL, 1, true, UNRECOGNIZED, to_parse, to_parse);
	return (to_parse + i);
}
...
int	verify_flag_value(char *flag_value)
{
...
int	verify_flag_identifier(char *flag_id)
{
	if (strcmp(flag_id, "m") == 0)
		return (0);
	if (strcmp(flag_id, "f") == 0)
		return (0);
	if (strcmp(flag_id, "q") == 0)
		return (0);
	if (strcmp(flag_id, "p") == 0)
		return (0);
	return (-1);
}
...
int	parse_flag(t_parameters *params, char **args, int argc, int index)
{
	...
	if (verify_flag_identifier(flag_id) == -1)
		error_exit(params, 1, true, INVALID_OPTION, flag_id);
	if (index == argc)
		error_exit(params, 1, true, MISSING_VALUE, flag_id);
	flag_value = args[index];
	if (verify_flag_value(flag_value) == -1)
		error_exit(params, 1, true, INVALID_VALUE, flag_id, flag_value);
...
void verify_packet_len(t_parameters *params)
{
	if (check_is_int(params->string_packet_len) == -1)
		error_exit(params, 1, true, INVALID_PACKETLEN, params->string_packet_len);
}
